...
namespace ft
{
    ...
    public:
        ...
        matrix_iterator(const matrix_iterator &cpy) : mat(0)
        {
            mat = cpy.mat;
            m = cpy.m;
            n = cpy.n;
        }
        matrix_iterator &operator=(const matrix_iterator &cpy)
        {
            mat = cpy.mat;
            m = cpy.m;
            n = cpy.n;
    ...
    public:
        ...
        matrix adj()
        {
            ...
            for (size_type i = 0; i < n; i++)
            {
                for (size_type j = 0; j < n; j++)
                {
                    ...
                    tmp.mat[i][j] = minor.det() * static_cast<float>((i + j) % 2 ? -1 : 1);
                }
            }
            return tmp;
        }
        ...
        matrix<T> inv()
        {
            ...
            // Extract the right half (inverse) of the augmented matrix
        ...
        friend std::ostream &operator<<(std::ostream &os, matrix mat)
        {
            ...
            for (size_type i = 0; i < mat.n; i++)
            {
                ...
                if (mat.m > 0)
                    os << std::setw(width) << mat[i][mat.m - 1];
                os << "]";
                if (i < mat.n - 1)
                    os << std::endl;
            }
        ...
        {
            *this *= matrix<float>({{cosf(theta), sinf(theta), 0, 0},
    ...
    matrix<float> eulerToRotation(float roll, float pitch, float yaw, uint n)
    {
        ...
        {
            ret = matrix<float>({{cosf(yaw) * cosf(pitch), cosf(yaw) * sinf(pitch) * sinf(roll) - sinf(yaw) * cosf(roll), cosf(yaw) * sinf(pitch) * cosf(roll) + sinf(yaw) * sinf(roll), 0},
                                 {sinf(yaw) * cosf(pitch), sinf(yaw) * sinf(pitch) * sinf(roll) + cosf(yaw) * cosf(roll), sinf(yaw) * sinf(pitch) * cosf(roll) - cosf(yaw) * sinf(roll), 0},
                                 {-sinf(pitch), cosf(pitch) * sinf(roll), cosf(pitch) * cosf(roll), 0},
                                 {0, 0, 0, 1}});
        }
        return ret;
    }
    ...
    vector<float> rotationToEuler(const matrix<float> &rotation)
    {
        ...
        float roll = atan2f(rotation[2][1], rotation[2][2]);
    ...
    matrix<float> rotate(matrix<float> m, float theta, vector<float> axis)
    {
        theta = -theta;
        axis = axis / norm(axis);
        return matrix<float>({{cosf(theta) + axis[0] * axis[0] * (1 - cosf(theta)), axis[0] * axis[1] * (1 - cosf(theta)) + axis[2] * sinf(theta), axis[0] * axis[2] * (1 - cosf(theta)) - axis[1] * sinf(theta), 0},
    ...
    matrix<float> camera(vector<float> const &eye, vector<float> const &center, vector<float> const &up)
    {
        ...
        return matrix<float>({{s[0], u[0], -f[0], 0},
                              {s[1], u[1], -f[1], 0},
                              {s[2], u[2], -f[2], 0},
